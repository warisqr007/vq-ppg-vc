...
from src.data_load import VqBnfDatasetNoDr, MultiSpkVcCollate, OneshotArciticVcDataset
from src.transformer_bnftomel_prosody_ecapa import Transformer
...
class Solver(BaseSolver):
    ...
    def load_data(self):
        ...
        train_dataset = VqBnfDatasetNoDr(
    ...
    def set_model(self):
        ...
        self.load_ckpt()
        self.load_pretrained_spk_embed_model()
    
    def load_pretrained_spk_embed_model(self):
        self.spk_embed_model = torch.hub.load('RF5/simple-speaker-embedding', 'gru_embedder', verbose=False)
        self.spk_embed_model.to(self.device)
    # @torch.no_grad()
    ...
    # @torch.no_grad()
    # def compute_speaker_embedding(self, mels):
    #     spk_ids = self.spk_embed_model(mels)
    #     torch.cuda.empty_cache()
    #     return spk_ids
    ...
    def exec(self):
        ...
        while self.step < self.max_step:
            for data in self.train_dataloader:
                ...
                # with torch.no_grad():
                #     spk_ids_tg = self.compute_speaker_embedding(mels)

                loss, _, after_outs, before_outs, ys, olens = self.model(
                    xs=ppgs,
                    ilens=in_lengths,
                    ys=mels,
                    olens=out_lengths,
                ...
                # with torch.no_grad():
                #     # spkdvs_out = self.compute_speaker_embedding(before_outs)
                #     spkdvs_out = self.compute_speaker_embedding(after_outs)
                ...
                # spkd_loss = F.cosine_similarity(spk_ids_tg, spkdvs_out, dim=1).mean()
                # # spkd_loss = mseloss(spk_ids_tg, spkdvs_out)
                # mel_loss = loss.cpu().item()
                # # print('spk-sim: ',spkd_loss.cpu().item())
                # spk_loss = 4*(1 - spkd_loss.cpu().item())
                # print('mel-loss: ',mel_loss)
                # print('spk-loss: ',spk_loss)
                # # print(lr_rate)
                # loss = loss + 4*(1-spkd_loss)
                self.timer.cnt("fw")
                ...
                if (self.step == 1) or (self.step % self.PROGRESS_STEP == 0):
                    ...
                    # self.progress("Tr stat | T Loss - {:.4f} | M Loss - {:.4f} | S Loss - {:.4f} | Grad. Norm - {:.2f} | {}"
                    #               .format(loss.cpu().item(), mel_loss, spk_loss, grad_norm, self.timer.show()))
    ...
    def validate(self):
        ...
        self.save_checkpoint(f'step_{self.step}.pth', 'loss', dev_loss, show_msg=True)
